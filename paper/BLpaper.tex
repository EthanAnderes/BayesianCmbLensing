...
The analog to the Ancillary toy problem is related to the lensed CMB problem as follows
\begin{align*}
...
Notice in the analog toy model the relation between the sufficient parameter $\widetilde t$ and the ancillary parameter $t$ is through the equation $\widetilde t - \varphi = t$. A similar type of relation hold in the lensed CMB problem which becomes key for sampling from both conditionals distributions (\ref{suff 1}) and (\ref{suff 2}). The analogy is what we are referring to as the {\em anti-lensing approximation}
\begin{equation}
\label{antilense}
 \widetilde T(x-\nabla\phi(x)) \approx T(x) \quad\text{\bf\em analogous to}\quad \widetilde t - \varphi = t.
\end{equation}
We distinguish between what we call {\em inverse lensing displacement}, which is the true coordinate displacement that maps $\tilde T \mapsto T$, and with {\em anti-lensing} which is  given by $-\nabla\phi$ and approximates inverse lensing. 
The inverse lensing dislacement is denoted
$-\nabla \phi^\text{inv}(x) - \nabla^\perp \psi^\text{inv}(x)$ where $\nabla^\perp \equiv \bigr(-\frac{\partial}{\partial y},\frac{\partial}{\partial x} \bigl)$ and  $\psi^\text{inv}$ denotes a stream function potential that models a field rotation.
In particular
\begin{align*}
...
{\em Remark 1: } Anti-lensing is a good approximation to inverse lensing:
\begin{align}
\label{anti approx}
-\nabla\phi(x) &\approx -\nabla\phi^\text{inv}(x) \\
&\approx -\nabla\phi^\text{inv}(x)- \nabla^\perp \psi^\text{inv}(x).
\end{align}
This is due too the fact that the expected size of the displacement $\nabla\phi$ is much smaller than the correlation length scale of $\phi$. Indeed, the typical displacement size $\nabla \phi(x)$ is less than 3 arcmin whereas the correlation length scale of $\phi$ is on the order of degrees. In Figure \ref{antilensing plots} we show a simulation of a lensing potential $\phi$ (upper left) with the true inverse lensing potential $\phi^\text{inv}$ (upper right). The difference $\phi - \phi^\text{inv}$ is also shown (bottom left) along with the stream function $\psi^\text{inv}$. Clearly,  the magnitude of the difference  $\phi^\text{inv}-\phi$ and $\psi^\text{inv}$ subdominant to estimation error. 


{\em Remark 2: } The Hamiltonian sampler, described below in Section \ref{hamiltonian sampler section}, which samples from the approximate anti-lensing potential posteriro $P(\phi | \widetilde T,  \text{\rm data})$ can be easily adjusted to sample from the true inverse lensing stream and potential functions $P(\phi^\text{inv},\psi^\text{inv} | \widetilde T,  \text{\rm data})$.   However, we have excluded it from our analysis since the magnitude of the lensing potential is at least an order of magnitude smaller than estimation error. 
...
\caption{The difference between anti-lensing and inverse lensing. {\em Upper left:} lensing potential $\phi$. {\em Upper right:} The inverse lensing potential $\phi^\text{inv}$. {\em Bottom left:} The difference $\phi^\text{inv}-\phi$. {\em Bottom right:} The inverse lensing stream function $\psi^\text{inv}$.}
\end{figure}
...
\label{hamiltonian sampler section}
