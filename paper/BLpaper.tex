...
Notice in the analog toy model the relation between the suffient parameter $\widetilde t$ and the ancillary parameter $t$ is through the equation $\widetilde t - \varphi = t$. A similar type of relation hold in the lensed CMB problem which becomes key for sampling from both conditionals distributions (\ref{suff 1}) and (\ref{suff 2}). The analogy is what we are referring to as the {\em anti-lensing approximation}
\begin{equation}
\label{antilense}
 \widetilde T(x-\nabla\phi(x)) \approx T(x) \quad\text{\bf\em analogous to}\quad \widetilde t - \varphi = t.
\end{equation}
There are two points to make here. First, anti-lensing is a good approximation to the inverse lensing operation $\tilde T(x)\rightarrow T(x)$. Second, in the methodology that we describe below, the anti-lensing approximation can be made exact with the introdution of a field rotation. 


Regarding the accuracy of the anti-lensing approximation...
 and is subdomanty to the esimtation uncertainty of the displacement field.
 This is do too the fact that the expected size of the displacement $\nabla\phi$ is much smaller than the correlation length scale of $\nabla\phi$. Indeed, the typoical displacement size $\nabla \phi(x)$ is less than 3 arcmin whereas the correlation length scale of $\phi$  is on the order of degrees. 


Regarding the field rotation, notice  one can introduce a field rotation parameter, call it $\psi$, which makes antilensing exact: $\widetilde T\bigl(x-\nabla\phi^a(x)+ \nabla^\perp \psi^a(x)\bigr) = T(x)$ where $\nabla^\perp \equiv \bigr(-\frac{\partial}{\partial y},\frac{\partial}{\partial x} \bigl)$.
 
We can quantify...this by showing the simluated Helmholts spectrums for the inverse lensign operation and show how small it is.
