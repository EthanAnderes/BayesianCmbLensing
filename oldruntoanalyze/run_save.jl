...
savepath = joinpath("simulations", "$seed") # savepath = joinpath("simulations", "test") 
simnotes = """
	* only consider phi_k up to l_max = 1000
	* only gradient updates at the beginning
	* the file directory gives the seed
	* after generating 10 samples, we do a large cooling update
"""
...
parhr = setpar(pixel_size_arcmin./hrfactor, hrfactor*n, beamFWHM, nugget_at_each_pixel, maskupC, maskupP);
...
run(`cp run.jl $savepath/run_save.jl`)
run(`cp src/makeQuicktime.py $savepath/makeQuicktime.py`) # this is so that I can make movies from the out dir
...
while true
	...
	tildetx_hr_curr[:] = spline_interp2(parhr.grd.x, parhr.grd.y, tx_hr_curr, parhr.grd.x + phidx1_hr, parhr.grd.y + phidx2_hr) # this one has the uniform noise
